...
type ChromaticSymbolsBackgroundProps = {
  className?: string
  color?: string // "r, g, b" format, e.g. "233, 201, 155"
}

export function ChromaticSymbolsBackground({
  className,
  color = "233, 201, 155",
}: ChromaticSymbolsBackgroundProps) {
  ...
  useEffect(() => {
    ...
    function createSimplexNoise() {
      ...
      return {
        noise3D(xin: number, yin: number, zin: number) {
          ...
          if (t0 < 0) n0 = 0.0
          else { t0 *= t0; n0 = t0 * t0 * (gi0 % 2 === 0 ? 1 : -1) * (x0 + y0) }

          let t1 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1
          if (t1 < 0) n1 = 0.0
          else { t1 *= t1; n1 = t1 * t1 * (gi1 % 2 === 0 ? 1 : -1) * (x1 + y1) }

          let t2 = 0.6 - x2 * x2 - y2 * y2 - z2 * z2
          if (t2 < 0) n2 = 0.0
          else { t2 *= t2; n2 = t2 * t2 * (gi2 % 2 === 0 ? 1 : -1) * (x2 + y2) }

          let t3 = 0.6 - x3 * x3 - y3 * y3 - z3 * z3
          if (t3 < 0) n3 = 0.0
          else { t3 *= t3; n3 = t3 * t3 * (gi3 % 2 === 0 ? 1 : -1) * (x3 + y3) }
    ...
    function handleMouseMove(e: MouseEvent) {
      ...
      mouse.y = (e.clientY - rect.top) * currentDpr
      mouse.isActive = true
    }
    ...
    function animate(timestamp: number) {
      ...
      const activeHoverRadius = config.hoverRadius * currentDpr
